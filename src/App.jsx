...
import { 
  ...
  Edit2, Info, MapPin, TrendingUp, Minus, Check, LogOut, Store, ArrowRight, Lock, Filter, ChevronRight
} from 'lucide-react';
import { 
...
export default function App() {
  ...
  useEffect(() => {
    ...
    return () => { unsubProducts(); unsubOrders(); unsubExpenses(); };
  }, [user]);
    ...
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-4 md:p-8">
            ...
            {activeTab === 'settings' && <SettingsView profile={shopProfile} user={user} onUpdate={(p) => setShopProfile(p)} showToast={showToast} />}
        </div>
      </main>
    </div>
  );
...
const InventoryView = ({ products, user, showToast }) => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in">
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={isEdit ? 'পণ্য আপডেট' : 'নতুন পণ্য যুক্ত করুন'}>
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <Input label="কেনা দাম" type="number" value={form.buyPrice} onChange={e=>setForm({...form,buyPrice:e.target.value})} required/>
                  <Input label="বেচা দাম" type="number" value={form.sellPrice} onChange={e=>setForm({...form,sellPrice:e.target.value})} required/>
                  <Input label="স্টক পরিমাণ" type="number" value={form.stock} onChange={e=>setForm({...form,stock:e.target.value})} required/>
                </div>
...
const POSView = ({ products, user, shopProfile, showToast }) => {
    ...
    const updateQty = (id, val) => {
      ...
      setCart(cart.map(i => i.id === id ? {...i, qty: Math.max(0, num)} : i));
    };
    ...
    const handleOrder = async () => {
        ...
        if(!customer.name || !customer.phone) return showToast("কাস্টমার তথ্য দিন", "error");
        ...
        try {
          ...
          setCart([]); 
          setCustomer({name:'', phone:'', address:'', deliveryCharge:'0', advance:'0'});
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom-4">
            <div className="lg:col-span-2 space-y-6">
                <div className="relative flex gap-3">
                  <div className="relative flex-1">
                    ...
                    <input type="text" placeholder="পণ্য খুঁজুন..." className="w-full pl-12 pr-4 py-3 rounded-2xl border border-slate-200 outline-none focus:border-indigo-500 bg-white shadow-sm transition-all" value={filter} onChange={e=>setFilter(e.target.value)}/>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5 overflow-y-auto pr-2 h-[calc(100vh-280px)] pb-20 scrollbar-hide">
                  {products.filter(p=>p.name.toLowerCase().includes(filter.toLowerCase())).map(p => (
                    <div key={p.id} onClick={()=>addToCart(p)} className="bg-white p-5 rounded-3xl border border-slate-100 cursor-pointer hover:shadow-2xl hover:border-indigo-300 transition-all group relative overflow-hidden flex flex-col justify-between h-40">
                        <div>
                          <h4 className="font-bold text-base mb-1 text-slate-800 line-clamp-2">{p.name}</h4>
                          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{p.fabric}</p>
                        </div>
                        <div className="flex justify-between items-end">
                          <p className="text-[10px] text-indigo-500 font-black uppercase">Stock: {p.stock}</p>
                          <p className="text-indigo-600 font-black text-2xl">৳{p.sellPrice}</p>
                        </div>
            ...
            <Card className="flex flex-col gap-6 shadow-2xl border-indigo-50 h-fit sticky top-4 max-h-[calc(100vh-120px)] overflow-y-auto">
                <h3 className="font-black text-lg border-b pb-4 flex items-center gap-2 text-slate-800"><ShoppingBag size={20} className="text-indigo-600"/> অর্ডার কার্ট ({cart.length})</h3>
                <div className="space-y-3 flex-1 overflow-y-auto min-h-[100px] max-h-[300px]">
                  {cart.map(i => (
                    <div key={i.id} className="bg-slate-50 p-3 rounded-2xl border border-slate-200 relative group transition">
                        <div className="flex justify-between text-xs mb-2"><strong>{i.name}</strong><button onClick={()=>removeFromCart(i.id)} className="text-rose-400 hover:text-rose-600 p-1"><X size={14}/></button></div>
                        <div className="flex justify-between items-center gap-2">
                            <select value={i.size} onChange={e=>{setCart(cart.map(item => item.id === i.id ? {...item, size: e.target.value} : item))}} className="text-[10px] border border-slate-200 p-1 rounded-lg font-bold bg-white">{SIZES.map(s=><option key={s} value={s}>{s}</option>)}</select>
                            <div className="flex items-center bg-white border rounded-xl px-1 h-8">
                                <button onClick={()=>updateQty(i.id, i.qty - 1)} className="w-8 text-indigo-600 font-black hover:bg-slate-50 rounded-lg">-</button>
                                <input type="number" value={i.qty} onChange={(e)=>updateQty(i.id, e.target.value)} className="w-8 text-center text-xs font-black outline-none border-none bg-transparent"/>
                                <button onClick={()=>updateQty(i.id, i.qty + 1)} className="w-8 text-indigo-600 font-black hover:bg-slate-50 rounded-lg">+</button>
                            </div>
                ...
                <div className="space-y-4 border-t pt-4">
                    ...
                    <div className="flex gap-2">{PAYMENT_METHODS.map(m => (<button key={m.id} onClick={() => setPaymentMethod(m.id)} className={`flex-1 p-2 text-[10px] font-black border rounded-xl flex flex-col items-center gap-1 transition-all ${paymentMethod === m.id ? 'bg-indigo-600 text-white shadow-lg scale-105 border-indigo-600' : 'bg-white text-slate-400 border-slate-100'}`}>{m.icon}{m.label}</button>))}</div>
                    <div className="bg-slate-900 text-white p-4 rounded-2xl space-y-1">
                      <div className="flex justify-between text-xs opacity-60 font-bold uppercase tracking-widest"><span>Total Bill:</span><span>৳{financials.total}</span></div>
                      <div className="flex justify-between text-xl font-black"><span>মোট বকেয়া:</span><span>৳{financials.due}</span></div>
                    </div>
                    <Button onClick={handleOrder} className="w-full py-4 shadow-xl text-lg font-black bg-indigo-600 hover:bg-indigo-700">অর্ডার কনফার্ম</Button>
                </div>
            </Card>
...
const OrderListView = ({ orders, user, shopProfile, showToast }) => {
    ...
    const handleUpdatePayment = async () => {
        ...
        try {
          await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'orders', editPayment.id), {
            deliveryCharge: del, 
    ...
    return (
        <div className="space-y-8 animate-in fade-in">
            ...
            <Card className="p-0 overflow-hidden shadow-md border-none">
              <div className="overflow-x-auto">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {orders.map(o => (
                      <tr key={o.id} className="hover:bg-slate-50/80 transition">
                        ...
                        <td className="p-5 text-right font-black text-slate-700">৳{o.totalAmount}</td>
                        <td className="p-5 text-right font-black text-rose-500">৳{o.dueAmount}</td>
                        <td className="p-5 text-center">
                          <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wide ${o.status === 'Paid' ? 'bg-emerald-100 text-emerald-700' : 'bg-orange-100 text-orange-700'}`}>{o.status}</span>
                        </td>
                        <td className="p-5 text-right flex gap-3 justify-end items-center mt-2">
                          ...
                          <button onClick={async ()=>{ if(confirm("ডিলিট করবেন?")) { await deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'orders', o.id)); showToast("অর্ডার ডিলিট হয়েছে"); } }} className="text-rose-400 hover:bg-rose-50 p-2 rounded-xl transition"><Trash2 size={18}/></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <Modal isOpen={!!editPayment} onClose={()=>setEditPayment(null)} title="পেমেন্ট আপডেট">
              <div className="space-y-5">
                ...
                <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 flex justify-between items-center">
                  <div><p className="text-[10px] text-indigo-400 font-bold uppercase">বর্তমান বকেয়া</p><p className="text-2xl font-black text-indigo-700">৳{editPayment?.dueAmount}</p></div>
                  <ChevronRight className="text-indigo-200" size={32}/>
                </div>
                <Input label="নতুন জমা পরিমাণ" type="number" value={payData.amount} onChange={e=>setPayData({...payData, amount: e.target.value})} placeholder="টাকা লিখুন"/>
                <Button onClick={handleUpdatePayment} className="w-full py-4 text-lg bg-indigo-600 hover:bg-indigo-700">জমা নিশ্চিত করুন</Button>
              </div>
            </Modal>
        </div>
    );
        ...
        <div className="fixed inset-0 bg-slate-900/80 z-[200] flex items-center justify-center p-4 backdrop-blur-md overflow-y-auto">
            <div className="bg-white rounded-3xl max-w-2xl w-full flex flex-col max-h-[90vh] shadow-2xl animate-in zoom-in">
                <div className="p-6 border-b flex justify-between items-center bg-slate-50 rounded-t-3xl">
                  <span className="font-black text-slate-700">Invoice Preview</span>
                ...
                <div className="overflow-y-auto p-12 bg-white" ref={printRef}>
                    <div className="border-b-4 border-slate-900 pb-8 mb-8 flex justify-between items-start">
                        <div>
                          <h1 className="text-4xl font-black uppercase text-indigo-700 tracking-tighter mb-2">{shopProfile?.shopName}</h1>
                          <p className="text-sm font-bold text-slate-500">{shopProfile?.address}</p>
                          <p className="text-sm font-bold text-slate-500">{shopProfile?.phone}</p>
                        </div>
                        <div className="text-right">
                          <h2 className="text-4xl font-black text-slate-200 tracking-[0.2em] uppercase">Invoice</h2>
                          <p className="font-black text-slate-800 mt-2 text-xl">#INV-{order.id?.slice(-6).toUpperCase()}</p>
                          <p className="text-[10px] text-slate-400 font-black uppercase mt-1">{dateStr}</p>
                        </div>
                    </div>
                    
                    <div className="mb-10 p-6 bg-slate-50 rounded-3xl border border-slate-100 grid grid-cols-2 gap-8">
                        <div>
                          ...
                          <p className="text-sm text-slate-600 font-bold">{order.phone}</p>
                        ...
                        <div className="text-right flex flex-col justify-center">
                          ...
                          <span className={`text-2xl font-black uppercase mt-1 ${order.status === 'Paid' ? 'text-emerald-600' : 'text-rose-600'}`}>{order.status}</span>
                        </div>
                    </div>

                    <table className="w-full mb-8"><thead className="bg-slate-900 text-white text-[10px] uppercase font-black tracking-widest"><tr><th className="p-4 text-left rounded-l-2xl">Item Desc</th><th className="p-4 text-center">Qty</th><th className="p-4 text-right">Price</th><th className="p-4 text-right rounded-r-2xl">Total</th></tr></thead>
                        <tbody className="divide-y divide-slate-100">
                          {order.items?.map((i,idx)=>(
                            <tr key={idx} className="text-sm"><td className="p-4 font-bold text-slate-700">{i.name} <span className="text-[10px] opacity-50 ml-2">({i.size})</span></td><td className="p-4 text-center font-black">{i.qty}</td><td className="p-4 text-right text-slate-500">৳{i.sellPrice}</td><td className="p-4 text-right font-black text-slate-800">৳{i.sellPrice * i.qty}</td></tr>
                          ))}
                        </tbody>
                    </table>

                    <div className="flex justify-end"><div className="w-72 space-y-3 bg-slate-50 p-8 rounded-3xl text-sm border border-slate-100">
                        <div className="flex justify-between text-slate-400 font-bold"><span>Sub-total</span><span>৳{order.subTotal}</span></div>
                        <div className="flex justify-between text-slate-400 font-bold pb-3 border-b border-slate-200"><span>Delivery</span><span>৳{order.deliveryCharge}</span></div>
                        <div className="flex justify-between text-2xl font-black text-slate-800 pt-2"><span>Total Bill</span><span>৳{order.totalAmount}</span></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in">
            <Card><h3 className="font-black mb-8 flex items-center gap-2 text-xl"><CreditCard size={24} className="text-rose-500"/> খরচ এন্ট্রি</h3>
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <Button type="submit" className="w-full py-4 mt-4 text-lg shadow-xl shadow-rose-100" variant="danger" icon={<Save size={20}/>}>খরচ সেভ করুন</Button>
                </form>
            </Card>
                    ...
                    {expenses.map(ex => (
                        <div key={ex.id} className="bg-white p-6 rounded-3xl border border-slate-100 flex justify-between items-center shadow-sm hover:shadow-xl transition group">
                            <div className="flex gap-4 items-center">
                              <div className="w-12 h-12 bg-rose-50 text-rose-500 rounded-2xl flex items-center justify-center font-bold text-xl">-</div>
                              <div><p className="font-bold text-slate-800 text-lg leading-none">{ex.title}</p><p className="text-[10px] text-slate-400 uppercase font-black tracking-widest mt-1">{ex.category}</p></div>
                            </div>
                            <div className="flex items-center gap-5">
                              <span className="text-rose-500 font-black text-2xl">৳{ex.amount}</span>
                              <button onClick={async ()=>{ if(confirm("মুছবেন?")) await deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'expenses', ex.id)); }} className="p-2 opacity-0 group-hover:opacity-100 hover:bg-rose-50 rounded-xl text-rose-400 transition"><Trash2 size={18}/></button>
                            </div>
                        </div>
                    ))}
                ...
                {customers.map((c, i) => (
                    <Card key={i} className="hover:shadow-2xl transition border border-slate-100 group relative">
                        <div className="flex items-start justify-between mb-4">
                            <div className="w-14 h-14 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl flex items-center justify-center text-white font-black text-2xl shadow-lg shadow-indigo-100">{c.name.charAt(0)}</div>
                            <span className="bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider">{c.totalOrders} Orders</span>
                        </div>
                        ...
                        <p className="text-xs text-slate-400 mt-2 line-clamp-1">{c.address || 'কোন ঠিকানা নেই'}</p>
                        <div className="pt-6 mt-6 border-t border-slate-50 flex justify-between items-center">
                            <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Lifetime Value</span>
                            <span className="text-xl font-black text-slate-800">৳{c.totalSpent.toLocaleString()}</span>
                        </div>
                    </Card>
                ))}
            </div>
    ...
    return (
        <div className="max-w-2xl mx-auto animate-in fade-in">
            <Card className="border-t-8 border-indigo-600 shadow-2xl overflow-hidden relative">
                <div className="absolute top-0 right-0 p-10 opacity-5 -mr-10 -mt-10"><Settings size={180}/></div>
                <div className="flex items-center gap-6 mb-12 border-b pb-8 relative z-10">
                  <div className="w-20 h-20 bg-indigo-50 rounded-3xl flex items-center justify-center text-indigo-600 border border-indigo-100 shadow-sm"><Store size={40}/></div>
                  <div><h2 className="text-3xl font-black text-slate-800 tracking-tight">ব্যবসায়িক প্রোফাইল</h2><p className="text-sm text-slate-500 font-bold mt-1">ইনভয়েসে এই তথ্যগুলো ব্যবহার হবে</p></div>
                </div>
...
const Select = ({ label, value, onChange, options }) => (
  <div className="flex flex-col gap-2 w-full">
    {label && <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">{label}</label>}
    <select value={value ?? ''} onChange={onChange} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-500 outline-none transition-all text-slate-800 bg-white text-sm cursor-pointer font-bold">
